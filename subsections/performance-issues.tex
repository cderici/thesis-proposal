...
such as the regexp implementation. However, since it now evaluates
Racket code to extend the functionalities of the interpreter, it is
...
\secref{subsec:rpython}, a language interpreter written in RPython can
utilize a \verb|JitDriver| reflection provided by the RPython
framework to define its own "program counter" and use hints such as
\verb|jit_merge_point| and \verb|can_enter_jit| to indicate a starting
point for a trace and where the interpreter might perform a backwards
jump, respectively. An example can be seen in
\figref{fig:rpython-interp} in \secref{subsec:rpython}.

Recall that the Pycket's interpreter uses the two-state tracking to
determine the trace header as a pair of a lambda AST form and its
call-site. In its original form in \figref{fig:regexp} (without the
meta-hints), in order for a trace to be formed for one of the looping
points in the \verb|match-pat| function, the input needs to go into
the same branch enough times to make that branch "hot". Therefore,
...
\begin{figure}[h!]
  ...
  \begin{mdframed}
\begin{lstlisting}[mathescape,escapechar=\%,language=lisp]
(%\underline{define/with-meta-hints}% (match-pat pattern p-pos str s-pos m)
  (cond
    [(>= p-pos (string-length pattern)) m]
    [(and (< (+ p-pos 1) (string-length pattern)) (char=? (string-ref pattern (+ p-pos 1)) #\*))
    ...
    [(char=? (string-ref pattern p-pos) #\.)
      (and (< s-pos (string-length str))
            (%\underline{can-enter-jit}% (match-pat pattern (add1 p-pos) str (add1 s-pos) (cons (string-ref str s-pos) m))))]
    [else
     (and (char=? (string-ref pattern p-pos) (string-ref str s-pos))
           (%\underline{can-enter-jit}% (match-pat pattern (add1 p-pos) str (add1 s-pos) (cons (string-ref str s-pos) m))))]))
\end{lstlisting}
...
The meta-hints approach aim to solve this problem by allowing the
RPython hints to be dynamically changed or added via the source
program. For example, \figref{fig:annotated-regexp} shows the main
dispatch-loop of the regexp implementation in \figref{fig:regexp},
annotated with meta-hints to dynamically modify Pycket's interpreter's
way of tracking the program state. With these annotations supported by
the language interpreter (possibly through primitives that internally
modify the AST states), the tracer knows that a point might be a
backwards jump and stops tracing further\footnote{Pycket's hints in
  the interpreter code would allow it to go further} and only
considers the path from the body entry through a particular branch to
the looping call. Note that the input is still a non-repetitive
sequence of data, so the tracer will still go in different branches at
each step. However, when a path for a branch becomes hot, the
generated trace will contain only the codes for that particular path,
thereby producing a tight and concise loop for that single branch.
