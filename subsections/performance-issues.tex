...
together. [FIXME: Figure asdasd] shows the language we're working on,
which is similar to the Racket Core language used in
...
from the source level. \figref{fig:cek-switch-stack} shows the
overview of the interaction between the two models. We can start with
either of the interpreters and switch back and forth between them
using the \racketcode{convert} forms manually within the source code.
...
\begin{figure}[h!]
  \centering
  \includegraphics[scale=0.5]{img/stackful}
\end{figure}
